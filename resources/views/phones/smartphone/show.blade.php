<x-layout>
    <x-slot name="heading">
        <h1 class="text-3xl font-bold mb-4">Attributes of {{ $smartphone->brands->name }} {{ $smartphone->models->name }}
        </h1>
    </x-slot>
    ...
    <div class="bg-gray-100 rounded-lg shadow-md p-8 mb-6">
        ...
        <p class="text-lg">
            {{ $smartphone->attributes($smartphone->brands->name, $smartphone->models->name, $smartphone->colors->name) }}
        </p>
    </div>

    <div class="bg-blue-500 rounded-lg shadow-md p-8 mb-6">
        <h2 class="text-lg font-bold mb-2 text-white">Price:</h2>
        <p class="text-lg text-white">${{ $smartphone->price }}</p>
    </div>
